...
def findTrafficSign():
    ...
    while True:
        ...
        cv2.imshow('mask',cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR))
        ...
        # if largestRect is not None:
            ...
            cv2.putText(frame, detectedTrafficSign, tuple(
                largestRect[0]), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 255, 0), 2)
...
def identifyTrafficSign(image):
    ...
    cv2.rectangle(image, (subWidth, 4 * subHeight),
                  (3 * subWidth, 9 * subHeight), (0, 255, 0), 2)  # left block
    cv2.rectangle(image, (4 * subWidth, 4 * subHeight),
                  (6 * subWidth, 9 * subHeight), (0, 255, 0), 2)  # center block
    cv2.rectangle(image, (7 * subWidth, 4 * subHeight),
                  (9 * subWidth, 9 * subHeight), (0, 255, 0), 2)  # right block
    cv2.rectangle(image, (3 * subWidth, 2 * subHeight),
                  (7 * subWidth, 4 * subHeight), (0, 255, 0), 2)  # top block
    ...
    leftFraction = np.sum(leftBlock) / \
        (leftBlock.shape[0] * leftBlock.shape[1])
    centerFraction = np.sum(centerBlock) / \
        (centerBlock.shape[0] * centerBlock.shape[1])
    rightFraction = np.sum(rightBlock) / \
        (rightBlock.shape[0] * rightBlock.shape[1])
